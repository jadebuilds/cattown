...
import sys, socket, select, errno, time
import threading
...
class KlipperInterface:
    ...
    def enqueue_gcode(self, gcode: GCode):
        """
        Send a 
        """
        self.webhook_socket.send(gcode.to_str().encode('ascii') + b"\x03")
    ...
    def _thread_run(self):
        while not self.should_close.is_set():
            self.poll.poll(1000.)  # indicates receive ready (is this necessary?)
